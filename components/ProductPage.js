...
export default function ProductGenerator() {
  ...
  const [ProductPageComp, setProductPageComp] = useState(null)

  const openPage = async () => {
    if (!ProductPageComp) {
      const mod = await import('./ProductPage')
      setProductPageComp(() => mod.default)
    }
    setShowPage(true)
  }
  ...
  return (
    <div>
      {showPage && ProductPageComp && (
        <ProductPageComp product={displayResult} onClose={() => setShowPage(false)} />
      )}
        ...
        <>
          <div className="card">
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 16 }}>
              <button className="btn btn-primary" onClick={openPage}>
