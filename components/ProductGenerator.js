...
export default function ProductPage({ product, onClose }) {
  ...
  const basePrice = parseFloat(String(product.price || '29.99').replace('€', '').replace(',', '.')) || 29.99

  const getBullets = (p) => {
    if (!p || !p.bullets) return []
    if (Array.isArray(p.bullets)) return p.bullets
    return String(p.bullets).split('\n').filter(b => b.trim())
  }
      ...
      <div className="product-page">
        ...
        <div className="pp-main">
          ...
          <div className="pp-info">
            ...
            <p className="pp-subtitle">{String(product.description || '').substring(0, 150)}...</p>
        ...
        <div className="pp-section">
          ...
          <div className="pp-benefits">
            {getBullets(product).map((bullet, i) => (
              <div key={i} className="pp-benefit">
                <span className="pp-benefit-icon">✓</span>
                <span>{String(bullet).replace('✓', '').trim()}</span>
              </div>
            ))}
          </div>
        </div>
